<template>
  <div>
    ...
    >
      ...
      ></el-table-column>
      <!-- receiverCode -->
      <el-table-column label="接收北斗卡号" prop="bdsNumber" align="center">
        <template> </template>
      </el-table-column>
      <el-table-column
        label="载体"
...
//查看参数
const lookFrom = ref({});
const bdsNumaber = ref("");
watch(
  () => props.bdsNumber,
  (val) => {
    if (val) {
      // queryParams.value.bdsNumber = val;
      bdsNumaber.value = val;
      getList();
    }
  },
  {
...
async function getList() {
  try {
    ...
    if (queryParams.value.msgType == 2 || !queryParams.value.msgType) {
      const origin = await 发送列表查询({
        ...queryParams.value,
        bdsNumber: bdsNumaber.value,
      });
