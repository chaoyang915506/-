<template>
  <chinaGeoCoordMap
    :chartData="chartData"
    :geoCoordMap="geoCoordMap"
    :chinaDatas="chinaDatas"
    :isCheckPath="isCheckPath"
    :isShowInfoWindow="isShowInfoWindow"
    :isMapOrSiteMap="isMapOrSiteMap"
    :showViewer="showViewer"
    :currentMoniter="currentMoniter"
    @checkImg="checkImg"
    @handleLineClick="handleLineClick"
    @handleCheckClick="handleCheckClick"
    @expcloseInfoWindow="expcloseInfoWindow"
    ref="map"
    style="width: 100%; height: 90vh"
  ></chinaGeoCoordMap>
  <el-image-viewer
    class="out_image_viewer_box"
    :z-index="ZIndex"
    v-if="showViewer"
    @close="closeViewer"
    :url-list="imgUrl"
  />
  <infoWindowCheck
    @handleCloseDialog="handleCloseDialog"
    @expcloseInfoWindow="expcloseInfoWindow"
    :isShowInfoWindow="isShowInfoWindow"
    @goBack="goBack"
    @getInitList="getInitList"
    :infoWindowData="infoWindowData"
  ></infoWindowCheck>
</template>
<script setup>
import chinaGeoCoordMap from "@/components/echarts/chinaGeoCoordMap.vue";
import siteGeoCoordMap from "@/components/echarts/siteGeoCoordMap.vue";
import infoWindowCheck from "@/views/display/handle/infoWindowCheck.vue";
import {
  getMapInformation,
  getLineMonitorState,
  getFilterMapInfor,
} from "@/api/display/display.js";
import { watch } from "@vue/runtime-core";

let props = defineProps({
  isMapOrSiteMap: {
    type: Boolean,
    default: true,
  },
  checkedLines: {
    type: Array,
    default: [],
  },
  currentMoniter: {
    type: Object,
    default: {},
  },
});
let emit = defineEmits(["handleIsShowLenged", "goBack"]);
let { proxy } = getCurrentInstance();
let chartData = ref([]);
let isCheckPath = ref(false);
let isShowInfoWindow = ref(false);
let infoWindowData = ref({});
let imgUrl = ref([]);
let ZIndex = ref(99999);
let showViewer = ref(false);
onMounted(() => {
  getInitList([]);
});
watch(
  () => props.checkedLines,
  () => {
    // console.log(props.checkedLines, 'checkedLines')
    getInitList(props.checkedLines);
  }
);
watch(
  () => props.currentMoniter,
  () => {
    // console.log(props.currentMoniter, "currentMoniter");
    if (props.currentMoniter) {
      handleLineClick(props.currentMoniter);
    } else {
      getInitList([]);
    }
  }
);
// 初始化地图数据
async function getInitList(ids) {
  let res;
  if (ids.length > 0) {
    res = await getFilterMapInfor(ids.join(","));
    console.log('first')
  } else {
    res = await getMapInformation();
  }

  if (res.code == 200) {
    let tempObj = {};
    let tempArr = {};
    emit("handleIsShowLenged", false);
    isCheckPath.value = false;
    if (res.data.data && res.data.data.length) {
      res.data.data.forEach((item) => {
        tempObj[item.lineId] = [];
      });
      res.data.data.forEach((item) => {
        tempObj[item.lineId].push(item);
      });
    }
    for (let key in tempObj) {
      tempObj[key] = tempObj[key].sort(
        (a, b) => a.serialNumber - b.serialNumber
      );
    }
    console.log(tempObj, 'tempObj')
    for (let key in tempObj) {
      tempObj[key].forEach((item) => {
        tempArr[key] = {
          name: item.lineStart,
          lineId: item.lineId,
          siteNameArr: tempObj[key].map((i) => {
            let reg = /\r\n/g;
            return i.siteName.replace(reg, "");
          }),
          sitesNum: item.sitesNum,
          lineName: item.lineName,
          lineEnd: item.lineEnd,
          path: [],
        };
      });
    }
    for (let key in tempObj) {
      tempObj[key].forEach((item) => {
        tempArr[key].path.push([item.lon, item.lat]);
      });
    }
    let arr = [];
    for (let key in tempArr) {
      arr.push(tempArr[key]);
    }
    // console.log(tempObj, "tempObj", tempArr, arr);
    chartData.value = arr;
  }
}
// 线路点击
async function handleLineClick(val) {
  // console.log(val, "线路点击");
  if (val.lineId == undefined) return;
  let res = await getLineMonitorState({
    lineId: val.lineId.toString(),
  });

  // console.log(res, "什么");
  if (res.data.length == 0) {
    return proxy.$modal.msgError("当前线路没有检测点");
  }
  let ret = await getFilterMapInfor(val.lineId.toString());
  if (res.code == 200) {
    emit("handleIsShowLenged", true);
    isCheckPath.value = true;
    // console.log(res.data, "res");
    if (res.data && res.data.length) {
      res.data.forEach((item) => {
        item.name = item.monitorName;
        item.lnglat = [item.lon, item.lat];
      });
      if (ret.code == 200 && ret.data.data && ret.data.data.length) {
        res.data.unshift({
          lnglat: [ret.data.data[0].lon, ret.data.data[0].lat],
          name: ret.data.data[0].lineStart,
        });
        res.data.push({
          lnglat: [
            ret.data.data[ret.data.data.length - 1].lon,
            ret.data.data[ret.data.data.length - 1].lat,
          ],
          name: ret.data.data[ret.data.data.length - 1].lineStart,
        });
      }
      let lonlatlist = ret.data.data.map((i) => {
        return [i.lon, i.lat];
      });
      
      // console.log(res.data);
      // console.log(ret.data.data, val, "rd");
      chartData.value = [
        {
          points: res.data,
          trainstatilist: lonlatlist,
          lineName: val.lineName,
          stationlist: ret.data.data.map((i) => {
            let reg = /\r\n/g;
            return i.siteName.replace(reg, "");
          }),
        },
      ];
    } else {
      return;
      chartData.value = [];
    }
    // console.log(chartData.value, "arrrrrr");
  }
}
const checkImg = (val) => {
  // imgUrl.value = ["data:image/png;base64," + val.file.fileBytes];
  imgUrl.value = [formatterImg(val)];
  showViewer.value = true;
};
const closeViewer = () => {
  showViewer.value = false;
};
// 查看详情
function handleCheckClick(val) {
  infoWindowData.value = val;
  isShowInfoWindow.value = true;
}
// 关闭弹框
function handleCloseDialog() {
  isShowInfoWindow.value = false;
}
// fomatter图片路径
const formatterImg = (item) => {
  // console.log(item.file.fileBytes, "imgurl");
  return item.file ? proxy.$arrayBufferToBlob(item.file.fileBytes) : "";
  // return new URL(imgUrl, import.meta.url).href;
};
// var geoCoordMap = ref({
//   '新疆玛纳斯基地': [86.22, 44.30],
//   '九江': [116.00, 29.70],
//   '新乡': [116.402217, 35.311657],
//   ' ': [79.92, 37.12],
//   '  ': [86.85, 47.70],
//   '若羌县': [88.17, 39.02],
//   '上海': [121.4648, 31.2891],
//   '东莞': [113.8953, 22.901],
//   '东营': [118.7073, 37.5513],
//   '中山': [113.4229, 22.478],
//   '临汾': [111.4783, 36.1615],
//   '临沂': [118.3118, 35.2936],
//   '丹东': [124.541, 40.4242],
//   '丽水': [119.5642, 28.1854],
//   '乌鲁木齐': [87.9236, 43.5883],
//   '佛山': [112.8955, 23.1097],
//   '保定': [115.0488, 39.0948],
//   '兰州': [103.5901, 36.3043],
//   '包头': [110.3467, 41.4899],
//   '北京': [116.4551, 40.2539],
//   '北海': [109.314, 21.6211],
//   '南京': [118.8062, 31.9208],
//   '南宁': [108.479, 23.1152],
//   '南昌': [116.0046, 28.6633],
//   '南通': [121.1023, 32.1625],
//   '厦门': [118.1689, 24.6478],
//   '台州': [121.1353, 28.6688],
//   '合肥': [117.29, 32.0581],
//   '呼和浩特': [111.4124, 40.4901],
//   '咸阳': [108.4131, 34.8706],
//   '哈尔滨': [127.9688, 45.368],
//   '唐山': [118.4766, 39.6826],
//   '嘉兴': [120.9155, 30.6354],
//   '大同': [113.7854, 39.8035],
//   '大连': [122.2229, 39.4409],
//   '天津': [117.4219, 39.4189],
//   '太原': [112.3352, 37.9413],
//   '威海': [121.9482, 37.1393],
//   '宁波': [121.5967, 29.6466],
//   '宝鸡': [107.1826, 34.3433],
//   '宿迁': [118.5535, 33.7775],
//   '常州': [119.4543, 31.5582],
//   '广州': [113.5107, 23.2196],
//   '廊坊': [116.521, 39.0509],
//   '延安': [109.1052, 36.4252],
//   '张家口': [115.1477, 40.8527],
//   '徐州': [117.5208, 34.3268],
//   '德州': [116.6858, 37.2107],
//   '惠州': [114.6204, 23.1647],
//   '成都': [103.9526, 30.7617],
//   '扬州': [119.4653, 32.8162],
//   '承德': [117.5757, 41.4075],
//   '拉萨': [91.1865, 30.1465],
//   '无锡': [120.3442, 31.5527],
//   '日照': [119.2786, 35.5023],
//   '昆明': [102.9199, 25.4663],
//   '杭州': [119.5313, 29.8773],
//   '枣庄': [117.323, 34.8926],
//   '柳州': [109.3799, 24.9774],
//   '株洲': [113.5327, 27.0319],
//   '武汉': [114.3896, 30.6628],
//   '汕头': [117.1692, 23.3405],
//   '江门': [112.6318, 22.1484],
//   '沈阳': [123.1238, 42.1216],
//   '沧州': [116.8286, 38.2104],
//   '河源': [114.917, 23.9722],
//   '泉州': [118.3228, 25.1147],
//   '泰安': [117.0264, 36.0516],
//   '泰州': [120.0586, 32.5525],
//   '济南': [117.1582, 36.8701],
//   '济宁': [116.8286, 35.3375],
//   '海口': [110.3893, 19.8516],
//   '淄博': [118.0371, 36.6064],
//   '淮安': [118.927, 33.4039],
//   '深圳': [114.5435, 22.5439],
//   '清远': [112.9175, 24.3292],
//   '温州': [120.498, 27.8119],
//   '渭南': [109.7864, 35.0299],
//   '湖州': [119.8608, 30.7782],
//   '湘潭': [112.5439, 27.7075],
//   '滨州': [117.8174, 37.4963],
//   '潍坊': [119.0918, 36.524],
//   '烟台': [120.7397, 37.5128],
//   '玉溪': [101.9312, 23.8898],
//   '珠海': [113.7305, 22.1155],
//   '盐城': [120.2234, 33.5577],
//   '盘锦': [121.9482, 41.0449],
//   '石家庄': [114.4995, 38.1006],
//   '福州': [119.4543, 25.9222],
//   '秦皇岛': [119.2126, 40.0232],
//   '绍兴': [120.564, 29.7565],
//   '聊城': [115.9167, 36.4032],
//   '肇庆': [112.1265, 23.5822],
//   '舟山': [122.2559, 30.2234],
//   '苏州': [120.6519, 31.3989],
//   '莱芜': [117.6526, 36.2714],
//   '菏泽': [115.6201, 35.2057],
//   '营口': [122.4316, 40.4297],
//   '葫芦岛': [120.1575, 40.578],
//   '衡水': [115.8838, 37.7161],
//   '衢州': [118.6853, 28.8666],
//   '西宁': [101.4038, 36.8207],
//   '西安': [109.1162, 34.2004],
//   '贵阳': [106.6992, 26.7682],
//   '连云港': [119.1248, 34.552],
//   '邢台': [114.8071, 37.2821],
//   '邯郸': [114.4775, 36.535],
//   '郑州': [113.4668, 34.6234],
//   '鄂尔多斯': [108.9734, 39.2487],
//   '重庆': [107.7539, 30.1904],
//   '金华': [120.0037, 29.1028],
//   '铜川': [109.0393, 35.1947],
//   '银川': [106.3586, 38.1775],
//   '镇江': [119.4763, 31.9702],
//   '长春': [125.8154, 44.2584],
//   '长沙': [113.0823, 28.2568],
//   '长治': [112.8625, 36.4746],
//   '阳泉': [113.4778, 38.0951],
//   '青岛': [120.4651, 36.3373],
//   '韶关': [113.7964, 24.7028]
// });
// var chinaDatas = ref([
//   [{
//     name: '新乡'
//   }, {
//     name: '新乡',
//     value: 200
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '呼和浩特',
//     value: 90
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '哈尔滨',
//     value: 90
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '石家庄',
//     value: 90
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '昆明',
//     value: 30
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '北京',
//     value: 100
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '长春',
//     value: 40
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '重庆',
//     value: 40
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '贵阳',
//     value: 50
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '南宁',
//     value: 30
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '济南',
//     value: 10
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '太原',
//     value: 40
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '西安',
//     value: 60
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '武汉',
//     value: 50
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '合肥',
//     value: 40
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '南京',
//     value: 30
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '沈阳',
//     value: 20
//   }],
//   [{
//     name: '新乡'
//   }, {
//     name: '成都',
//     value: 10
//   }]
// ])
let map = ref(null);
//关闭小弹框
const expcloseInfoWindow = () => {
  map.value.expcloseInfoWindow();
};
//返回首页
const goBack = () => {
  emit("goBack");
};
defineExpose({ getInitList, expcloseInfoWindow });
</script>
<style scoped>
</style>