		...
		<el-form label-width="80px" :inline="true" size="normal">
			...
			<el-form-item>
				...
				<el-select
					v-else
					clearable
					v-model="queryParams.weatherState"
					@change="getList"
				>
					<el-option
			...
			<div v-if="pic_data == '0'" class="piclist_sum">
				<div class="list_pic" v-for="i in allDataList" :key="i.id">
					<div class="pic_body" @click="viewDetails(i)">
						...
						<div class="shade">
							<div>
								<span>材质类别: {{ i.weatherState }}</span>
							</div>
							<div>
								<span>异物描述: {{ i.environmentDescription }}</span>
							</div>
							<div>
				...
				<el-col :span="12">
					...
					>
						...
						<el-form-item label="气象状态">
							<span class="detail_style">{{ formData.weatherState }}</span>
						</el-form-item>
...
import {
	...
	deletSurr,
	getWeatherCount,
} from "@/api/simulation/compTest/surroundings";
import echartsPictorialBar from "@/components/EchartsPictorialBar/index.vue";
const { proxy } = getCurrentInstance();
const { texture, scene_type } = proxy.useDict("texture", "scene_type");
//日期选择器
const getDate = (e) => {
	if (!e) {
...
function SelectChange() {
	data.queryParams.weatherState = "";
	data.queryParams.endDate = "";
	data.queryParams.startDate = "";
	data.timePicker = [];
}
//全部的异物数据
getSurrCount().then((res) => {
...
getWeatherCount().then((res) => {
	...
	data.WeatherDisaster.xAxisData = arr.map((i) => i.name);
	data.WeatherDisaster.seriesData = arr.map((i) => i.count);
});
//查数据
...
const submitForm = async () => {
	...
	let res = await addSurr(formData);
	cancel();
	if (res.code === 200) {
