...
import { delSameValue } from "@/utils/delSameObjValue.js";
...
// 		});
// 		let WindStatusArr = delSameValue(val, "label", "value");
// 		data.WeatherDisaster.xAxisData = WindStatusArr.flat().map((i) => i.label);
// 		data.WeatherDisaster.seriesData = WindStatusArr.flat().map((i) => i.value);
// 	}
// );
//选择线路好
const lineChange = (e) => {
...
/** 删除按钮操作 */
function deleteAll() {
	if (data.sesetionData.length === 0)
		return proxy.$modal.msgError("请选择检测设备！");
...
function formatterList(data, arr, keyname) {
	let keylist = arr.map((i) => {
		const findData = data.find((id) => id.value == i.weatherType);
		//为了页面好看后期改成空或者天气
		return {
			name: findData ? findData.label : "无",
			count: i.weatherCount,
		};
	});
	//处理字典值多个返回大风
	let obj = {};
