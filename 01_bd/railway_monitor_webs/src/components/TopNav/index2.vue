...
// 所有的路由信息
const routers = computed(() => store.state.permission.topbarRouters2)
...
function handleSelect(key, keyPath) {
    ...
    if (isHttp(key)) {
        ...
        //activeRoutes(key)
        if (activeRoutes(key) && activeRoutes(key).length) {
            router.push({
                path: activeRoutes(key)[0].children
                    ? activeRoutes(key)[0].path + '/' + activeRoutes(key)[0].children[0].path
        ...
        // router.push({
        //     path: activeRoutes(key) && activeRoutes(key).length ? activeRoutes(key)[0].path : '/index',
        // })
    }
}
